...
router.post('/', requireRole('applicant'), (req, res) => {
  const { posting_id, cover_message } = req.body
  const posting = get('SELECT * FROM postings WHERE id = ?', posting_id)
  ...
  run('INSERT INTO status_history (application_id, status, note) VALUES (?, ?, ?)', id, 'submitted', 'Your application was submitted')
...
router.get('/company', requireRole('company'), (req, res) => {
...
function loadFullApplication(id) {
  return get(
...
router.get('/:id', (req, res) => {
  const row = loadFullApplication(req.params.id)
  if (!row) return res.status(404).json({ error: 'Application not found' })
  if (!canAccess(req, row)) return res.status(403).json({ error: 'Not your application' })

  const history = all('SELECT * FROM status_history WHERE application_id = ? ORDER BY id ASC', row.id)
  row.status_history = history
  res.json(serializeApplication(row))
})

router.put('/:id/status', requireRole('company'), (req, res) => {
  const row = loadFullApplication(req.params.id)
  if (!row) return res.status(404).json({ error: 'Application not found' })
  if (!canAccess(req, row)) return res.status(403).json({ error: 'Not your application' })
  ...
  run('UPDATE applications SET status = ?, updated_at = ? WHERE id = ?', status, nowTs(), row.id)
  const note = String(req.body?.note || '').trim()
  run('INSERT INTO status_history (application_id, status, note) VALUES (?, ?, ?)', row.id, status, note)

  const updated = loadFullApplication(row.id)
  updated.status_history = all('SELECT * FROM status_history WHERE application_id = ? ORDER BY id ASC', row.id)
  res.json(serializeApplication(updated))
})

router.post('/:id/withdraw', requireRole('applicant'), (req, res) => {
  const row = loadFullApplication(req.params.id)
  if (!row) return res.status(404).json({ error: 'Application not found' })
  ...
  run('UPDATE applications SET status = ?, updated_at = ? WHERE id = ?', 'withdrawn', nowTs(), row.id)
  run('INSERT INTO status_history (application_id, status, note) VALUES (?, ?, ?)', row.id, 'withdrawn', 'Application withdrawn by applicant')
